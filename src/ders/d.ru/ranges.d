...
$(P
$(IX reduce, std.algorithm) Alternatively, the length may be calculated with less code by taking advantage of $(C std.algorithm.reduce). $(C reduce()) takes an operation as its template parameter and applies that operation to all of the elements of a range:
)
...
$(P
The $(C a) in the template parameter represents the current result ($(I the sum) in this case) and $(C b) represents the current element. The first function parameter is the initial value of the result ($(C size_t.init) is 0) and the second function parameter is the range that contains the elements.
)
