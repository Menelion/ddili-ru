...
$(P
As with $(C map()), it helps to explain $(C reduce()) from the $(C std.algorithm) module first.
)

$(P
$(IX reduce, std.algorithm) We have seen $(C std.algorithm.reduce) in the $(LINK2 /ders/d.en/ranges.html, Ranges chapter) before. $(C reduce()) is another high-level algorithm commonly found in many functional languages. Just like $(C map()), it takes one or more functions as template parameters. As its function parameters, it takes a value to be used as the initial value of the result, and a range. $(C reduce()) calls the functions with the current value of the result and each element of the range. When no initial value is specified, the first element of the range is used instead.
)
