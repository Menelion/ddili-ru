...
$(P
All of the $(C  bool) values above are automatically printed as "false" or "true". It is not the case in the opposite direction: the strings "false" and "true" are not automatically read as the values $(C false) and $(C true). For that reason, the input must be first read as a string and then be converted to a $(C bool) value.
)

$(P
Since one of the exercises below requires you to enter "false" and "true" from the standard input, I have been forced to use D features that I haven't explained to you yet. I will have to define a function below that will convert the input string to a $(C bool) value. This function will achieve its task by calling $(C to), which is defined in the module $(C std.conv). (You may see $(C ConvException) errors if you enter anything other than "false" or "true".)
)

$(P
I am hoping that all of the pieces of code that are in the $(C main()) functions of the following programs are clear at this point. $(C read_bool()) is the function that contains new features. Although I have inserted comments to explain what it does, you can ignore that function for now. Still, it must be a part of the source code for the program to compile and work correctly.
)
