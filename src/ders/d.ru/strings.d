...
$(H5 $(IX readln) $(IX chomp) $(C readln) and $(C chomp), instead of $(C readf))
...
$(P
Such control characters that are at the ends of strings can be removed by $(C std.string.chomp):
)
...
$(P
The $(C chomp()) expression above returns a new string that does not contain the trailing control characters. Assigning that return value back to $(C name) produces the intended output:
)
...
$(P
$(C readln()) can be used without a parameter. In that case it $(I returns) the line that it has just read. Chaining the result of $(C readln()) to $(C chomp()) enables a shorter and more readable syntax:
)
